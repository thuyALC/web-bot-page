...
## Neu YouTube bao khong phai bot

Render/VPS co the bi YouTube chan va bao:

```text
Sign in to confirm you're not a bot
```

Khi do can them cookies YouTube vao Render Environment Variables:

```text
YOUTUBE_COOKIES_B64=chuoi_base64_cua_file_cookies_txt
```

Cach lam nhanh tren may Windows:

1. Cai extension Chrome/Edge `Get cookies.txt LOCALLY`.
2. Dang nhap YouTube tren trinh duyet.
3. Export cookies cua `youtube.com` ra file `youtube-cookies.txt` dang Netscape.
4. Mo PowerShell tai thu muc co file cookies va chay:

```powershell
[Convert]::ToBase64String([IO.File]::ReadAllBytes("youtube-cookies.txt")) | Set-Clipboard
```

5. Vao Render -> Environment -> them `YOUTUBE_COOKIES_B64`, dan chuoi vua copy.
6. Save va Manual Deploy lai.

Khong nen public cookies nay len GitHub.
